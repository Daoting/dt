...
#region 引用命名
using Demo.Base.Sqlite;
using Microsoft.UI.Xaml;
using 主表X = Demo.Base.Sqlite.主表X;
using 基础X = Demo.Base.Sqlite.基础X;
using 大儿X = Demo.Base.Sqlite.大儿X;
using 小儿X = Demo.Base.Sqlite.小儿X;
using 扩展1X = Demo.Base.Sqlite.扩展1X;
using 扩展2X = Demo.Base.Sqlite.扩展2X;
using 父表X = Demo.Base.Sqlite.父表X;
#endregion

namespace Demo.Crud
{
    public partial class SqliteAccessDemo : Win
    {
        Random _rnd = new Random();

        public SqliteAccessDemo()
        {
            InitializeComponent();
        }
