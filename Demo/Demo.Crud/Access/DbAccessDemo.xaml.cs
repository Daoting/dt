...
namespace Demo.Crud
{
    public partial class DbAccessDemo : Win
    {
        ...
        #region 普通增删改查
        const string _sqlQuery = "select * from crud_基础";
        ...
        {
            var cnt = await AtPg.Exec("insert into crud_基础 (id, name, dispidx, mtime, enable_insert_event, enable_name_changed_event, enable_del_event) values (@id, @name, @dispidx, @mtime, true, true, true)",
        ...
        {
            long id = await AtPg.GetScalar<long>("select id from crud_基础 order by id limit 1");
            var cnt = await AtPg.Exec("update crud_基础 set name=name||'+' where id=@id", new { id = id });
            _tbInfo.Text = cnt > 0 ? "更新成功" : "更新失败";
        }

        async void OnDelPg(object sender, RoutedEventArgs e)
        {
            long id = await AtPg.GetScalar<long>("select id from crud_基础 order by id desc limit 1");
            var cnt = await AtPg.Exec("delete from crud_基础 where id=@id", new { id = id });
        ...
        async void OnInsert(object sender, RoutedEventArgs e)
        {
            ...
            At.SetDb("mydt");
            x = await 基础X.New("单个" + _rnd.Next(10000).ToString());
            suc = await x.Save();
            _tbInfo.Text += suc ? "\r\nmysql插入成功" : "\r\nmysql插入失败";
        ...
        {
            At.SetDb("pgdt");
            var x = await 基础X.First(_sqlFirst);
            if (x != null)
            {
                x.名称 += "+";
        ...
        async Task<Table<基础X>> GetBatchTbl()
        {
