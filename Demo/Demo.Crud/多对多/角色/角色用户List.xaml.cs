...
namespace Demo.Crud
{
    public partial class 角色用户List : List
    {
        public 角色用户List()
        {
            InitializeComponent();
        ...
        protected override async Task OnQuery()
        {
            if (_parentID > 0)
            {
                _lv.Data = await 用户X.Query($"where exists ( select user_id from crud_用户角色 b where a.ID = b.user_id and role_id={_parentID} )");
            }
            else
            {
                _lv.Data = null;
        ...
        async void OnAddRelated(Mi e)
        {
            ...
            if (await dlg.Show(_parentID.Value, e))
            {
                ...
                {
                    var x = new 用户角色X(UserID: row.ID, RoleID: _parentID.Value);
                    ls.Add(x);
                }
        ...
        async void OnDelRelated(Mi e)
        {
            ...
            else
            {
                ...
                if (row != null)
                    ls = new List<用户角色X> { new 用户角色X(UserID: row.ID, RoleID: _parentID.Value) };
            }
