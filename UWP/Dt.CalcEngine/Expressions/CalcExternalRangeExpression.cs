...
namespace Dt.CalcEngine.Expressions
{
    ...
    /// </summary>
    public class CalcExternalRangeExpression : CalcExternalExpression
    {
        ...
        public CalcExternalRangeExpression(ICalcSource source, int startRow, int startColumn, int endRow, int endColumn) : this(source, startRow, startColumn, endRow, endColumn, false, false, false, false)
        {
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="T:Dt.CalcEngine.Expressions.CalcExternalRangeExpression" /> class.
        /// </summary>
        /// <param name="source">The owner of cell range.</param>
        ...
        public CalcExternalRangeExpression(ICalcSource source, int startBandIndex, int endBandIndex, bool startBandRelative, bool endBandRelative, bool isRowBand) : base(source)
        {
        ...
        /// Return a <see cref="T:Dt.CalcEngine.Expressions.CalcExpression" /> which offset from current expression.
        /// </returns>
        public virtual CalcExternalRangeExpression Offset(int row, int column, int endRow, int endColumn, bool offsetAbsolute = false, bool offsetRelative = true)
        {
            ...
            switch (this.RangeType)
            {
                case CalcRangeType.Cell:
                    return new CalcExternalRangeExpression(base.Source, this.StartRow + (flag ? row : 0), this.StartColumn + (flag2 ? column : 0), this.EndRow + (flag3 ? endRow : 0), this.EndColumn + (flag4 ? endColumn : 0), this.StartRowRelative, this.StartColumnRelative, this.EndRowRelative, this.EndColumnRelative);

                case CalcRangeType.Row:
                    return new CalcExternalRangeExpression(base.Source, this.StartRow + (flag ? row : 0), this.EndRow + (flag3 ? endRow : 0), this.StartRowRelative, this.EndRowRelative, true);

                case CalcRangeType.Column:
                    return new CalcExternalRangeExpression(base.Source, this.StartColumn + (flag2 ? column : 0), this.EndColumn + (flag4 ? endColumn : 0), this.StartColumnRelative, this.EndColumnRelative, false);

                case CalcRangeType.Sheet:
                    return new CalcExternalRangeExpression(base.Source);
            }
            return new CalcExternalRangeExpression(base.Source, this.StartRow + (flag ? row : 0), this.StartColumn + (flag2 ? column : 0), this.EndRow + (flag3 ? endRow : 0), this.EndColumn + (flag4 ? endColumn : 0), this.StartRowRelative, this.StartColumnRelative, this.EndRowRelative, this.EndColumnRelative);
        }
