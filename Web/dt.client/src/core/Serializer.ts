...
export default class Serializer {
  ...
  static serialize(p_value: any): any {
    ...
    // 内置类型
    if (tp === "[object Table]") return Serializer.serializeTable(p_value);
    if (tp === "[object Row]") return Serializer.serializeRow(p_value);
    if (tp === "[object Dict]") return Serializer.serializeDict(p_value);

    // 数组列表
    if (tp === "[object Array]") return Serializer.serializeArray(p_value);
   ...
   * @returns
   */
  private static serializeTable(tbl: Table): any[] {
    const arr: any[] = [];
    // 类型
    arr.push("#tbl");
   ...
   * @returns
   */
  private static serializeChildRow(row: Row): any[] {
    const arr: any[] = [];
   ...
   * @returns
   */
  private static serializeRow(row: Row): any[] {
    const arr: any[] = [];
    arr.push("#row");
  ...
  static deserialize(p_jsonObj: any): any {
    ...
    if (
      ...
      if (first.startsWith("#")) {
        if (alias === "tbl") return Serializer.deserializeTable(p_jsonObj);
        if (alias === "dict") return Serializer.deserializeDict(p_jsonObj);
        if (alias === "row") return Serializer.deserializeTable(p_jsonObj);
        if (alias === "msg") return Serializer.deserializeTable(p_jsonObj);
        if (alias === "letter") return Serializer.deserializeTable(p_jsonObj);
        if (alias === "object")
