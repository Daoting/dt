...
{
    /// <summary>
    /// Specifies the series axis of the chart.
    /// </summary>
    public class ExcelChartSeriesAxis : IExcelChartSeriesAxis, IExcelChartAxis
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="T:Dt.Xls.Chart.ExcelChartSeriesAxis" /> class.
        /// </summary>
        public ExcelChartSeriesAxis()
        {
            this.Type = ExcelChartAxisType.Series;
        ...
        internal void ReadXml(XElement node, MemoryFolder mFolder, XFile xFile)
        {
            foreach (XElement element in node.Elements())
            {
                ...
                {
                    this.MajorTickMark = this.ReadTickMark(element.GetAttributeValueOrDefaultOfStringType("val", "none"));
                }
                else if (element.Name.LocalName == "minorTickMark")
                {
                    this.MinorTickMark = this.ReadTickMark(element.GetAttributeValueOrDefaultOfStringType("val", "none"));
                }
        ...
        {
            writer.WriteStartElement("c", "serAx", null);
            ...
            if ((!string.IsNullOrWhiteSpace(this.NumberFormat) && (this.NumberFormat != "General")) || !this.NumberFormatLinked)
            {
