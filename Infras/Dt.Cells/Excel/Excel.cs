...
namespace Dt.Base
{
    ...
    public partial class Excel : Panel, IXmlSerializable
    {
        ...
        /// <returns></returns>
        public Task SaveXmlAsync(Stream xmlStream, bool dataOnly = false)
        {
            // WinUI
        ...
        /// <returns></returns>
        public Task SaveCSV(int sheetIndex, Stream stream, TextFileSaveFlags flags = TextFileSaveFlags.None)
        {
            if ((sheetIndex < 0) || (sheetIndex >= SheetCount))
                throw new ArgumentOutOfRangeException("sheetIndex");
            // WinUI
            return RunTask(() => Sheets[sheetIndex].SaveCsv(stream, flags, Encoding.UTF8));
        }
        ...
        /// <param name="cellDelimiter">Cell delimiter string.</param>
        /// <returns></returns>
        public Task SaveTextFileRangeAsync(int sheetIndex, int row, int column, int rowCount, int columnCount, Stream stream, TextFileSaveFlags flags, string rowDelimiter, string columnDelimiter, string cellDelimiter)
        {
            if ((sheetIndex < 0) || (sheetIndex >= SheetCount))
                throw new ArgumentOutOfRangeException("sheetIndex");
            // WinUI
            return RunTask(() => Sheets[sheetIndex].SaveTextFileRange(row, column, rowCount, columnCount, stream, flags, rowDelimiter, columnDelimiter, cellDelimiter));
        }
        #endregion
