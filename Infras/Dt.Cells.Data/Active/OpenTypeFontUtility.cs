...
namespace Dt.Cells.Data
{
    ...
    internal class OpenTypeFontUtility : IMeasureable
    {
        ...
        internal BaseFont GetFont(Font font)
        {
            ...
            var fontName = font.FontFamilyName;
            if (string.IsNullOrEmpty(fontName))
            {
                font = (Font)font.Clone();
                font.FontFamily = this.defaultFont.FontFamily;
                fontName = font.FontFamilyName;
            }

            string str = $"{fontName},{font.FontStyleType}";
            ...
            else if (fontName.EndsWith("#DtIcon"))
            {
                // hdt 支持 DtIcon 字体，嵌入pdf
                // 全称："ms-appx:///icon.ttf#DtIcon"
                arialMT = OpenTypeFont.GetDtIconFont();
            }
            else
            {
                switch (fontName)
            ...
            {
                ExternalFontEventArgs args = new ExternalFontEventArgs(fontName);
                this.ExternalFont(this, args);
                byte[] data = args.FontData;
