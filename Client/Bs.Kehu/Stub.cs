...
namespace Dt.Shell
{
    ...
    public class Stub : IStub
    {
        ...
        {
            // 更新打开模型库
            //string error = await AtApp.OpenModelDb("kehu");
            ...
            // 安装后初次打开，显示向导
            string guide = AtLocal.GetCookie("ShowGuide");
            if (string.IsNullOrEmpty(guide))
            {
                p_info.LoadRootUI(new GuidePage());
                AtLocal.SaveCookie("ShowGuide", "false");
                return;
            }
        ...
        /// 获取状态库版本号，和本地不同时自动更新
        /// </summary>
        public string StateDbVer => "df131eadf7b2b35154459306bacf64bf";
        #endregion
    }
}
