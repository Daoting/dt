...
namespace Dt.Base.ListView
{
    ...
    internal class LvDataView
    {
        ...
        IList GetTransformedList()
        {
            ...
            {
                _owner.FilterCfg.PrepareFilter(_data[0]);
                ls = from item in ls
        ...
        IList GetTransformedGroup()
        {
            ...
            foreach (var group in _data.OfType<IList>())
            {
                var query = group.AsQueryable();

                // 1. 先执行where linq 过滤
                if (!string.IsNullOrEmpty(_owner.Where))
                {
                    // 只有转成实际类型，才能调用属性和方法
                    query = query.Cast(group[0].GetType()).Where(_owner.Where);
                }
                ...
                if (_owner.ExistFilterCfg)
                {
                    _owner.FilterCfg.PrepareFilter(group[0]);
                    ls = from item in ls
                         where _owner.FilterCfg.DoFilter(item)
                         select item;
                }
                ...
                if (_owner.SortDesc != null
                    && !string.IsNullOrEmpty(_owner.SortDesc.ID))
                {
                    if (group is Table tbl)
                    {
                        ls = ls.Order(new RowComparer(_owner.SortDesc, null));
                    }
                    else
                    {
                        var pi = group[0].GetType().GetProperty(_owner.SortDesc.ID, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
                        ls = ls.Order(new RowComparer(_owner.SortDesc, pi));
                    }
                }
                ...
                if (objs.Count > 0)
                {
                    var newGrp = new GroupData<object>(group.ToString(), objs);
                    result.Add(newGrp);
                }
            }
            return result;
