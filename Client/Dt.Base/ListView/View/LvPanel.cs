...
namespace Dt.Base.ListView
{
    ...
    /// Lv布局面板基类
    /// </summary>
    public abstract partial class LvPanel : Panel, IFilterHost
    {
        /************************************************************************************************************************************/
        // 平台调用顺序不同：
        ...
        double GetFixedTopHeight()
        {
            ...
            if (_groupHeader != null)
                height += _groupHeader.DesiredSize.Height;
            if (_filterUI != null)
                height += _filterUI.DesiredSize.Height;
            if (_toolbar != null)
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            if (_filterUI != null)
            {
                ...
                top += _filterUI.DesiredSize.Height;
            }
        ...
        protected void CreateFilterCfg()
        {
        ...
        Table IFilterHost.GetAllCols(string p_settingCols)
        {
            ...
            if (_owner.View is Cols cols)
            {
                ...
                {
                    bool ch = string.IsNullOrEmpty(p_settingCols) || p_settingCols.Contains(col.ID, StringComparison.OrdinalIgnoreCase);
                    tbl.AddRow(new
                    {
                        id = col.ID,
                        title = string.IsNullOrEmpty(col.Title) ? col.ID : col.Title,
                        ischecked = ch
                    });
                }
            }
            else if (_owner.View is DataTemplate temp)
            {
            ...
            else if (_owner.Data is Nl<Nav> || _owner.Data is Nl<GroupData<Nav>>)
            {
                tbl.AddRow(new { id = "Title", title = "标题", ischecked = true });
                tbl.AddRow(new { id = "Desc", title = "描述", ischecked = true });
            }
            return tbl;
        }
        ...
        {
            if (_owner.Data != null && _owner.Data.Count > 0)
            {
                var obj = _owner.Data[0];
