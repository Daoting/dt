...
namespace Dt.Base.ListView
{
    ...
    public partial class TablePanel : LvPanel
    {
        ...
        {
            // 列头
            _colHeader.Measure(new Size(Math.Max(0, _maxSize.Width - _topLeftWidth), _maxSize.Height));
            ...
            {
                // 无数据时按列头宽度
                _finalWidth = _colHeader.DesiredSize.Width + _topLeftWidth;
            }
        ...
        {
            // 只附加一次
            _owner.Cols.ColWidthChanged += () => InvalidateMeasure();
            ...
            var btn = new Tray { Icon = Icons.汉堡 };
            ToolTipService.SetToolTip(btn, "更多功能");
            btn.Click += ShowMenu;
            bd.Child = btn;
            _topLeft = bd;
            
            if (_owner.ColsCfg != null)
            {
                _owner.ColsCfg.CfgChanged += () => btn.Notice();
            }
        }
        ...
        internal async void OnAutoSaveColsChanged()
        {
            if (_owner.AutoSaveCols)
            {
                var cfg = new ColsCfg(_owner);
                await cfg.Init();
                _owner.ColsCfg = cfg;
            }
            else
        ...
        void ShowMenu(Tray e)
        {
            if (_menu == null)
            {
                _menu = new Menu { IsContextMenu = true, Placement = MenuPosition.TopRight };

                var mi = new Mi { ID = "筛选　　Ctrl+F", Icon = Icons.漏斗 };
                mi.Call += () =>
                {
                ...
                mi = new Mi { ID = "复制行　Ctrl+C", Icon = Icons.复制 };
                mi.Call += () => _owner.CopySelection();
                _menu.Items.Add(mi);

                if (_owner.SelectionMode == SelectionMode.Multiple)
                {
                    _menu.Items.Add(new MiSplit());
                    mi = new Mi { ID = "全选　　Ctrl+A", Icon = Icons.全选 };
                    mi.Call += () => _owner.SelectAll();
                    _menu.Items.Add(mi);

                    mi = new Mi { ID = "清除所选", Icon = Icons.取消选择 };
                    mi.Call += () => _owner.ClearSelection();
                    _menu.Items.Add(mi);
                }

                if (_owner.ShowReportMenu)
                {
                    _menu.Items.Add(new MiSplit());
                    mi = new Mi { ID = "预览报表", Icon = Icons.折线图 };
                    mi.Call += () => _owner.ShowReport(null, false, true);
                    _menu.Items.Add(mi);

                    mi = new Mi { ID = "导出Excel", Icon = Icons.Excel };
                    mi.Call += () => _ = _owner.ExportExcel(null, null, true);
                    _menu.Items.Add(mi);

                    mi = new Mi { ID = "导出Pdf", Icon = Icons.Pdf };
                    mi.Call += () => _ = _owner.ExportPdf(null, null, true);
                    _menu.Items.Add(mi);

                    mi = new Mi { ID = "打印", Icon = Icons.打印 };
                    mi.Call += () => _owner.Print(null, true);
                    _menu.Items.Add(mi);
                }
                ...
                {
                    _menu.Items.Add(new MiSplit());
                    mi = new Mi { ID = "恢复列默认布局", Icon = Icons.撤消 };
                    mi.Call += async () =>
                    {
                        if (_owner.ColsCfg != null)
                        {
                            bool suc = await _owner.ColsCfg.ResetCfg();
                            if (suc)
                            {
                                Reload();
                            }
