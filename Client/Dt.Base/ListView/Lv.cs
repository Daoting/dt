...
namespace Dt.Base
{
    ...
    public partial class Lv : Control
    {
        ...
        public static readonly DependencyProperty AutoSaveColsProperty = DependencyProperty.Register(
            "AutoSaveCols",
            typeof(bool),
            typeof(Lv),
            new PropertyMetadata(false, OnAutoSaveColsChanged));
        ...
        static void OnAutoSaveColsChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            Lv lv = (Lv)d;
            if (lv._panel is TablePanel tp)
            {
                tp.OnAutoSaveColsChanged();
            }
        }
        #endregion
        ...
        /// <summary>
        /// 表格视图时是否自动保存列设置，包括调整后的列宽、列序、隐藏列，以后显示时自动加载，默认false
        /// </summary>
        public bool AutoSaveCols
        {
            get { return (bool)GetValue(AutoSaveColsProperty); }
            set { SetValue(AutoSaveColsProperty, value); }
        }
        ...
        internal ColsCfg ColsCfg { get; set; }
