...
namespace Dt.Base.TreeDiagrams
{
    ...
    /// 树节点面板
    /// </summary>
    public partial class TdPanelItem : Panel
    {
        #region 成员变量
        ...
        #region 构造方法
        public TdPanelItem(TreeDiagram p_owner, TdItem p_row)
        {
            _owner = p_owner;
            _row = p_row;
            LoadContent();
        ...
        public void SetItem(TdItem p_item, bool p_isAsync)
        {
            if (_row == p_item)
                return;
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            var size = new Size(usedWidth + elem.DesiredSize.Width, height);
            ...
            return new Size(size.Width + 2 * _margin, size.Height);
        }
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            elem = (UIElement)Children[index++];
            elem.Arrange(new Rect(left, 0, elem.DesiredSize.Width, finalSize.Height));

            // 上下文菜单，垂直居中
            if (_btnMenu != null)
            {
                index++;
                _btnMenu.Arrange(new Rect(finalSize.Width - _btnWidth - _margin, (finalSize.Height - _btnWidth) / 2, _btnWidth, _btnWidth));
            }

            // 选择背景
            Rect rc = new Rect(_margin, 0, finalSize.Width - 2 * _margin, finalSize.Height);
            elem = (UIElement)Children[index++];
            elem.Arrange(rc);
        ...
        void LoadContent()
        {
            ...
            if (_owner.SelectionMode == SelectionMode.Multiple)
            {
                ...
                tbCheck.SetBinding(TextBlock.TextProperty, new Binding
                {
                    ...
                    Converter = new TreeViews.IsSelectedIconConverter(),
                });
            ...
            // 分割线及选择背景
            Border bd = new Border { IsHitTestVisible = false };
            if (_owner.SelectionMode != SelectionMode.None)
            {
                bd.SetBinding(Border.BackgroundProperty, new Binding
                {
                    Path = new PropertyPath("IsSelected"),
                    Converter = new TreeViews.SelectedBackgroundConverter(),
                });
            }
            Children.Add(bd);
