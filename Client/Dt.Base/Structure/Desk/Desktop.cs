...
namespace Dt.Base
{
    ...
    public partial class Desktop : Control
    {
        ...
        void LoadTaskbar()
        {
            ...
            _titleBarRightMargin = Math.Ceiling((double)bar.RightInset / XamlRoot.RasterizationScale);
            var margin = new Thickness(0, 0, _titleBarRightMargin, 0);
            if (tray != null)
                tray.Margin = margin;
            else
        ...
        void UpdateTaskbar()
        {
            double startWidth = _gridTaskbar.ColumnDefinitions[0].ActualWidth + _gridTaskbar.ColumnDefinitions[1].ActualWidth;
            double taskbarWidth = _gridTaskbar.ColumnDefinitions[2].ActualWidth;

            // _taskbarPanel.ActualWidth不准！
            double pnlWidth = taskbarWidth - _taskbarPanel.Margin.Right;
            if (Items.Count > 0)
            {
            ...
            if (Items.Count > 0)
            {
                leftWidth += ((TaskbarItem)_taskbarPanel.Children[0]).Width * Items.Count;
            }

            var scale = XamlRoot.RasterizationScale;
            var height = _taskbarPanel.ActualHeight;

            Windows.Graphics.RectInt32 rect = new(
                0,
                0,
                (int)Math.Ceiling(leftWidth * scale),
                (int)Math.Ceiling(height * scale));
            List<Windows.Graphics.RectInt32> ls = new();
            ls.Add(rect);

            var trayWidth = _gridTaskbar.ColumnDefinitions[3].ActualWidth;
            if (trayWidth > 0 && trayWidth > _titleBarRightMargin)
            {
                rect = new(
                    (int)Math.Ceiling((startWidth + taskbarWidth) * scale),
                    0,
                    (int)Math.Ceiling((trayWidth - _titleBarRightMargin) * scale),
                    (int)Math.Ceiling(height * scale));
                ls.Add(rect);
            }
            ...
            nonClient.SetRegionRects(Microsoft.UI.Input.NonClientRegionKind.Passthrough, ls.ToArray());
#endif
        }
