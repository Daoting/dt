...
namespace Dt.Core.Sqlite
{
    class SqliteCommandEx : SqliteCommand
    {
            ...
            where TEntity : Entity
        {
            ...
            // 列定义
            var tpEntity = typeof(TEntity);
            if (tpEntity.GetInterface("IVirEntity") == typeof(IVirEntity))
            {
                // 虚拟实体
                var tps = tpEntity.GetGenericArguments();
                foreach (var tp in tps)
                {
                    var map = SqliteConnectionEx.GetMapping(tp);
                    foreach (var col in map.Columns)
                    {
                        if (!tbl.Columns.Contains(col.Name))
            ...
            {
                // 普通实体
                var map = SqliteConnectionEx.GetMapping(tpEntity);
                foreach (var col in map.Columns)
                {
                    tbl.Add(col.Name, col.ColumnType);
                }
                ...
                while (reader.Read())
                {
                    ...
                    for (int i = 0; i < reader.FieldCount; i++)
                    {
                        ...
                        else
                        {
                            ...
                            var col = tbl.Columns[name];
                            if (reader.IsDBNull(i))
                                new Cell(row, name, col.Type);
                            else
                                new Cell(row, name, col.Type, reader.GetValue(i));
                        }
                    }
                    tbl.Add(row);
                }
            ...
            where TRow : Row
        {
            ...
            Func<string, TableMapping.Column> callback = null;
            var tpEntity = typeof(TRow);
            if (tpEntity.IsSubclassOf(typeof(Entity)))
            {
                if (tpEntity.GetInterface("IVirEntity") == typeof(IVirEntity))
                {
                    // 虚拟实体
                    List<TableMapping> maps = new List<TableMapping>();
                    var tps = tpEntity.GetGenericArguments();
                    foreach (var tp in tps)
                    {
                        maps.Add(SqliteConnectionEx.GetMapping(tp));
                    }
                    callback = name =>
                    {
                ...
                {
                    // 普通实体
                    var map = SqliteConnectionEx.GetMapping(typeof(TRow));
                    callback = name => map.FindColumn(name);
                }
            }
