...
namespace Dt.Mgr
{
    ...
    public static class AtWf
    {
        ...
        public static async void OpenFormWin(long p_itemID = -1, long p_prciID = -1, string p_prcName = null)
        {
            ...
            var formType = Kit.GetTypeByAlias(typeof(WfFormAttribute), info.PrcDef.Name);
            if (formType == null)
                Throw.Msg($"未指定流程表单类型，请在流程表单类型上添加 [WfForm(\"{info.PrcDef.Name}\")] 标签！");
            if (!formType.IsSubclassOf(typeof(WfForm)))
                Throw.Msg("任务表单类型需要继承自WfForm！");

            var form = (WfForm)Activator.CreateInstance(formType);
            await form.Query(info);
        }
