...
{
    public partial class GroupRoleList : List
    {
        ...
        protected override async Task OnQuery()
        {
            if (_parentID > 0)
            {
                _lv.Data = await RoleX.ExistsInGroup(_parentID.Value);
            }
            else
            {
                _lv.Data = null;
        ...
        async void OnAddRelated(Mi e)
        {
            ...
            if (await dlg.Show(_parentID.Value, e)
                && await RbacDs.AddGroupRoles(_parentID.Value, dlg.SelectedIDs))
            {
                await Refresh();
            }
        }
        ...
        async void OnDelRelated(Mi e)
        {
            ...
            if (ids != null && ids.Count > 0)
            {
                ...
                if (await RbacDs.RemoveGroupRoles(_parentID.Value, ids))
                    await Refresh();
            }
        }
    }
