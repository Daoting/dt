...
{
    public partial class UserRoleList : List
    {
        public UserRoleList()
        {
            InitializeComponent();
        ...
        protected override async Task OnQuery()
        {
            if (_parentID > 0)
            {
                _lv.Data = await RoleX.Query($"where exists ( select role_id from cm_user_role b where a.ID = b.role_id and user_id={_parentID.Value} )");
            }
            else
            {
                _lv.Data = null;
        ...
        async void OnAddRelated(Mi e)
        {
            ...
            if (await dlg.Show(_parentID.Value, e)
                && await RbacDs.AddUserRoles(_parentID.Value, dlg.SelectedIDs))
            {
                await Refresh();
            }
        }
        ...
        async void OnDelRelated(Mi e)
        {
            ...
            if (ids != null && ids.Count > 0)
            {
                ...
                if (await RbacDs.RemoveUserRoles(_parentID.Value, ids))
                    await Refresh();
            }
        }
    }
