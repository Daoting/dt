...
namespace Dt.Mgr.Rbac
{
    public partial class UserGroupList : Tab
    {
        ...
        public async void Refresh()
        {
            if (_releatedID > 0)
            {
                _lv.Data = await GroupX.Query("where exists ( select GroupID from cm_user_group b where a.ID = b.GroupID and UserID=@ReleatedID )", new Dict { { "ReleatedID", _releatedID.ToString() } });
            }
            else
            {
                _lv.Data = null;
