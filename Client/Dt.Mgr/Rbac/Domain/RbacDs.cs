...
namespace Dt.Mgr.Rbac
{
    public partial class RbacDs : DomainSvc<RbacDs>
    {
        ...
        public static async Task<List<long>> GetUsersOfRoles(List<long> p_roleIDs)
        {
            ...
            return await At.FirstCol<long>(string.Format(Sql角色列表的用户, string.Join(',', p_roleIDs)));
        }

        public static async Task<List<long>> GetUsersOfRole(long p_roleID)
        {
            return await At.FirstCol<long>(string.Format(Sql角色的用户, p_roleID));
        }
        ...
        public static async Task<bool> ExistsUserRole(long p_userID, long p_roleID)
        {
            ...
            return await At.GetScalar<int>($"select count(*) from cm_user_role where user_id={p_userID} and role_id={p_roleID}") > 0;
        }
        #endregion
        ...
        static async Task<bool> EditGroupRoles(long p_groupID, List<long> p_roleIDs, bool p_isRemove)
        {
            ...
            if (suc)
            {
                var users = await GroupX.GetRelationUserIDs(p_groupID);
        ...
        static async Task<bool> EditRoleGroups(long p_roleID, List<long> p_groupIDs, bool p_isRemove)
        {
            ...
            if (suc)
            {
                var users = await At.FirstCol<long>(string.Format(Sql分组列表的用户, string.Join(',', p_groupIDs)));
                DelUserDataVer(users);
        ...
        public static async void DelRoleDataVer(List<long> p_roleIDs, string p_key)
        {
            ...
            {
                // 包含任何人
                ls = await At.FirstCol<long>("select id from cm_user");
            }
            else
            {
        ...
        const string Sql角色的用户 = @"select user_id from cm_user_role where role_id={0}";
        #endregion
    }
}