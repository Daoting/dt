...
{
    public partial class RoleGroupList : List
    {
        ...
        protected override async Task OnQuery()
        {
            if (_parentID > 0)
            {
                _lv.Data = await GroupX.Query($"where exists ( select group_id from cm_group_role b where a.id = b.group_id and role_id = {_parentID} )");
            }
            else
            {
                _lv.Data = null;
        ...
        {
            var dlg = new Group4Role();
            if (await dlg.Show(_parentID.Value, e)
                && await RbacDs.AddRoleGroups(_parentID.Value, dlg.SelectedIDs))
            {
                await Refresh();
            }
        }
        ...
        async void OnDelRelated(Mi e)
        {
            ...
            if (ids != null && ids.Count > 0)
            {
                ...
                if (await RbacDs.RemoveRoleGroups(_parentID.Value, ids))
                    await Refresh();
            }
        }
    }
