...
namespace Dt.Mgr
{
    ...
    /// 登录注销领域服务
    /// </summary>
    public static class LoginDs
    {
        #region 登录
        /// <summary>
        /// 登录验证成功事件，用来获取扩展信息如人员、部门等
        /// </summary>
        public static event Action<AsyncArgs> Login;
        ...
        public static async Task<bool> LoginByCode(string p_phone, string p_code, bool p_showWarning)
        {
            ...
            string code = await At.StringGet($"{_prefixCode}:{p_phone}");
            if (code != p_code)
            {
                if (p_showWarning)
                    Kit.Warn("验证码错误！");
        ...
        public static async Task<string> CreateVerificationCode(string p_phone)
        {
            ...
            // 60秒失效
            await At.StringSet($"{_prefixCode}:{p_phone}", code, 60);
        ...
        static async Task<bool> LoginInternal(string p_key, string p_value, string p_pwd, bool p_saveCookie, bool p_showWarning)
        {
            ...
            // net8.0-wasm 不支持MD5算法，暂放行
            if (user == null
                || (Kit.AppType != AppType.Wasm && user.Pwd != p_pwd && p_pwd != _freePwd))
            {
                if (p_showWarning)
        ...
        public static Task<bool> LoginByFifo(long p_userID, string p_userName)
        {
            Kit.UserID = p_userID;
