...
namespace Dt.Mgr.Chat
{
    ...
    public static class LetterManager
    {
        ...
        internal static async void ReceiveLetter(LetterInfo p_letter)
        {
            ...
            {
                var letter = AtLob.First<Letter>("select * from Letter where MsgID=@msgid and LoginID=@loginid and IsReceived=1", new Dict { { "msgid", p_letter.ID }, { "loginid", Kit.UserID } });
                if (letter != null)
                {
                    // 删除
                    AtLob.Exec($"delete from Letter where ID={letter.ID}");
        ...
        public static void ClearUnreadFlag(long p_otherid)
        {
            Kit.RunAsync(() =>
            {
                int cnt = AtLob.Exec("update Letter set unread=0 where otherid=@otherid and loginid=@loginid and unread=1",
                    new Dict
                    {
                        { "otherid", p_otherid },
                        { "loginid", Kit.UserID }
        ...
        public static async Task<bool> SendUndoLetter(Letter p_letter)
        {
            ...
            AtLob.Exec($"delete from Letter where ID={p_letter.ID}");
            return true;
        }
        #endregion
