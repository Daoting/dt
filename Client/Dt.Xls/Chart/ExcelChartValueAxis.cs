...
namespace Dt.Xls.Chart
{
    ...
    /// </summary>
    public class ExcelChartValueAxis : IExcelChartValueAxis, IExcelChartAxis
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="T:Dt.Xls.Chart.ExcelChartValueAxis" /> class.
        /// </summary>
        public ExcelChartValueAxis()
        {
            this.Type = ExcelChartAxisType.Value;
            this.CrossBetween = Dt.Xls.Chart.CrossBetween.Between;
        ...
        internal void ReadXml(XElement node, MemoryFolder mFolder, XFile xFile)
        {
            foreach (XElement element in node.Elements())
            {
                ...
                else if (element.Name.LocalName == "crossBetween")
                {
                    switch (element.GetAttributeValueOrDefaultOfStringType("val", "between"))
                    {
                        case "between":
                            this.CrossBetween = Dt.Xls.Chart.CrossBetween.Between;
                            break;
                        ...
                        case "midCat":
                            this.CrossBetween = Dt.Xls.Chart.CrossBetween.MidpointOfCategory;
                            break;
                    }
                }
                else if (element.Name.LocalName == "majorUnit")
                {
                    ...
                    this.MajorUnit = element.GetAttributeValueOrDefaultOfDoubleType("val", 0.0);
                }
                else if (element.Name.LocalName == "minorUnit")
                {
                    this._isAutoMinorUnit = false;
                    this.MinorUnit = element.GetAttributeValueOrDefaultOfDoubleType("val", 0.0);
                }
                else if (element.Name.LocalName == "dispUnits")
                {
                    DisplayUnits units = new DisplayUnits();
                    units.ReadXml(element, mFolder, xFile);
                    this.DislayUnits = units;
                }
            }
        }
        ...
        {
            writer.WriteStartElement("c", "valAx", null);
            ...
            writer.WriteAttributeString("formatCode", this.NumberFormat);
            writer.WriteAttributeString("sourceLinked", this.NumberFormatLinked ? "1" : "0");
            ...
            if (this.CrossBetween == Dt.Xls.Chart.CrossBetween.Between)
            {
                writer.WriteLeafElementWithAttribute("crossBetween", null, "c", "val", "between");
            }
            else if (this.CrossBetween == Dt.Xls.Chart.CrossBetween.MidpointOfCategory)
            {
                writer.WriteLeafElementWithAttribute("crossBetween", null, "c", "val", "midCat");
            }
            if (!this._isAutoMajorUnit)
            {
                writer.WriteLeafElementWithAttribute("majorUnit", null, "c", "val", ((double) this.MajorUnit).ToString((IFormatProvider) CultureInfo.InvariantCulture));
            }
            if (!this._isAutoMinorUnit)
            {
                writer.WriteLeafElementWithAttribute("minorUnit", null, "c", "val", ((double) this.MinorUnit).ToString((IFormatProvider) CultureInfo.InvariantCulture));
            }
            if (this.DislayUnits != null)
            {
                this.DislayUnits.WriteXml(writer, mFolder, chartFile);
            }
            writer.WriteEndElement();
        }
        ...
        /// The cross between.
        /// </value>
        public Dt.Xls.Chart.CrossBetween CrossBetween { get; set; }
        ...
        /// The dislay units.
        /// </value>
        public DisplayUnits DislayUnits { get; set; }
