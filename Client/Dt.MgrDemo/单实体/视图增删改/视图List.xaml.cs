...
{
    public partial class 视图List : Tab
    {
        #region 构造方法
        public 视图List()
        {
        ...
        public async void Update()
        {
            if (Clause == null)
            {
                _lv.Data = await ChildViewX.Query(null);
            }
            else
            {
                var par = await Clause.Build<ChildViewX>();
                _lv.Data = await ChildViewX.Query(par.Sql, par.Params);
    }
        }
        #endregion
        ...
        async void OnDel(object sender, Mi e)
        {
            ...
            if (_lv.SelectionMode == SelectionMode.Multiple)
            {
                var ls = _lv.SelectedItems.Cast<ChildViewX>().ToList();
                if (await ls.Delete())
                {
                    Update();
