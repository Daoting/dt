...
namespace Dt.CalcEngine.Expressions
{
    ...
    /// </summary>
    public class CalcExternalCellExpression : CalcExternalExpression
    {
        private int _column;
        ...
        public CalcExternalCellExpression(ICalcSource source, int row, int column) : this(source, row, column, false, false)
        {
        }
        ...
        {
            return new CalcExternalCellIdentity(base.Source, this._rowRelative ? CalcHelper.NormalizeRowIndex(this.Row + row, 0xfffff) : this.Row, this._columnRelative ? CalcHelper.NormalizeRowIndex(this.Column + column, 0xfffff) : this.Column);
        }
        ...
        public override CalcExpression Offset(int row, int column, bool offsetAbsolute = false, bool offsetRelative = true)
        {
            ...
            return new CalcExternalCellExpression(base.Source, this.Row + (flag ? row : 0), this.Column + (flag2 ? column : 0), this.RowRelative, this.ColumnRelative);
        }
