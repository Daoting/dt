    ...
    public sealed partial class SamplesMain : Win
    {
        ...
        void LoadModuleList()
        {
            ...
            group = new GroupData<Nav>
            {
                ...
                new Nav("通用搜索视图", typeof(SearchMvWin)) { Desc = "包括固定搜索项、历史搜索项、统一搜索事件、统一导航等功能" },
                new Nav("单表模板", typeof(MyEntityWin)) { Desc = "单表增删改模板" },
                new Nav("多对多模板", typeof(ModuleView.MainWin)) { Desc = "多对多增删改模板" },
                new Nav("一对多，三栏", typeof(ModuleView.OneToMany1.ShoppingWin)) { Desc = "一对多增删改模板" },
                new Nav("一对多，两栏", typeof(ModuleView.OneToMany2.ShoppingWin)) { Desc = "一对多增删改模板" },
            };
            group.Title = "模块视图";
            ds.Add(group);
            #endregion
            ...
            group = new GroupData<Nav>
            {
                ...
                new Nav("文件", typeof(FileHome)) { Desc = "跨平台文件选择、上传下载文件、不同类型图像资源" },
                new Nav("数据访问与异常", typeof(DataAccessHome)) { Desc = "包括数据表操作、数据序列化、远程/本地数据的增删改查、远程过程调用等" },
                new Nav("切换到默认主页") { Callback = OpenHomeWin, Desc = "平台提供的默认主页，需要登录后才可加载" },
            };
            group.Title = "综合";
            ds.Add(group);
            #endregion
