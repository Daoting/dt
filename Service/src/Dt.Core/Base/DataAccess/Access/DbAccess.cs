...
namespace Dt.Core
{
    ...
    /// </summary>
    abstract class DbAccess : IDataAccess
    {
        #region 成员变量
        ...
        internal DbAccessInfo _dbInfo;
        #endregion

        #region 构造方法
        public DbAccess(DbAccessInfo p_info)
        {
            _dbInfo = p_info;
        }
        #endregion
        ...
        public async Task<int> BatchExec(List<Dict> p_dts)
        {
            ...
            try
            {
                ...
                foreach (Dict dt in p_dts)
                {
                    ...
                    if (dt.TryGetValue("params", out var pars))
                    {
                        if (pars is List<Dict> ls)
                        {
                            // 一个sql，多行数据
                            cnt += await Exec(sql, ls);
                        }
                        else if (pars is Dict par)
                        {
                            // 一个sql，一行数据
        ...
        {
            return Task.FromResult(_dbInfo.DbType);
        }

        public Task<string> GetDbKey()
        {
            return Task.FromResult(_dbInfo.Name);
        }

#if !SERVER
        public IAccessInfo AccessInfo => _dbInfo;
#endif
        #endregion
        ...
        public Task<string> StringGet(string p_key)
        {
            if (!string.IsNullOrEmpty(p_key))
                return GetScalar<string>($"select * from cm_cache where id='{p_key}'");
            return Task.FromResult(string.Empty);
        }

        public async Task<bool> StringSet(string p_key, string p_value, double p_seconds = 0)
        {
            if (string.IsNullOrEmpty(p_key))
                return false;
            ...
            return await BatchExec(ls) > 0;
        }

        public async Task<bool> KeyDelete(string p_key)
        {
            if (!string.IsNullOrEmpty(p_key))
                return await Exec($"delete from cm_cache where id='{p_key}'") > 0;
            return false;
        }
        ...
        public Task BatchKeyDelete(List<string> p_keys)
        {
            ...
            {
                ls.Add(new Dict { { "id", key } });
            }
            string prefix = _dbInfo.DbType == DatabaseType.Oracle ? ":" : "@";
            return Exec($"delete from cm_cache where id={prefix}id", ls);
        }
        #endregion
    }
}