...
namespace Dt.Core
{
        ...
        where TEntity : Entity
    {
        ...
        public static async Task<bool> DelByID(object p_id, bool p_directDel = true, bool p_isNotify = true)
        {
            ...
            if (!p_directDel)
            {
                ...
                if (_isVirEntity)
                {
                    ...
                    ew = new EntityWriter(vm.AccessInfo.GetDa());
                }
                else
                {
                    var model = await EntitySchema.Get(typeof(TEntity));
                    ew = new EntityWriter(model.AccessInfo.GetDa());
                }
#endif
        ...
        static async Task<bool> DelDirect(IList p_ids, bool p_isNotify = true)
        {
            ...
            if (dt["params"] is Dict par)
            {
                suc = await model.AccessInfo.GetDa().Exec((string)dt["text"], par) > 0;
            }
            else
            {
                suc = await model.AccessInfo.GetDa().BatchExec(new List<Dict> { dt }) > 0;
            }
        ...
        static async Task<bool> DelVirEntityDirect(IList p_ids, bool p_isNotify = true)
        {
            ...
            bool suc = await m.AccessInfo.GetDa().BatchExec(ls) > 0;
        ...
        /// <param name="p_colName">字段名或表达式，如 max(id)</param>
        /// <param name="p_where">where子句</param>
        /// <param name="p_params">参数值，支持Dict或匿名对象，默认null</param>
        /// <returns></returns>
        public static async Task<T> GetScalar<T>(string p_colName, string p_where, object p_params = null)
        {
#if SERVER
            var sql = await GetScalarSql(p_colName, p_where);
            return await Kit.DataAccess.GetScalar<T>(sql, p_params);
#else
            var res = await GetScalarSql(p_colName, p_where);
...
#if SERVER
        static async Task<string> GetScalarSql(string p_colName, string p_where)
        {
            string sql;
            if (_isVirEntity)
            {
                sql = (await VirEntitySchema.Get(typeof(TEntity))).GetScalarSql(p_colName);
            }
            else
            {
                sql = (await EntitySchema.Get(typeof(TEntity))).Schema.GetScalarSql(p_colName);
            }
...
#else
        static async Task<(IDataAccess, string)> GetScalarSql(string p_colName, string p_where)
        {
            string sql;
            IDataAccess da;
            if (_isVirEntity)
            {
                var vm = await VirEntitySchema.Get(typeof(TEntity));
                sql = vm.GetScalarSql(p_colName);
                da = vm.AccessInfo.GetDa();
            }
            else
            {
                var model = await EntitySchema.Get(typeof(TEntity));
                sql = model.Schema.GetScalarSql(p_colName);
                da = model.AccessInfo.GetDa();
            }
            ...
            if (!string.IsNullOrWhiteSpace(p_where))
            {
                ...
                {
                    Throw.Msg("GetScalar方法的参数以where开头！");
                }
            }
            return (da, sql);
        }
        ...
        public static async Task<long> NewID()
        {
            ...
            if (_isVirEntity)
            {
                da = (await VirEntitySchema.Get(typeof(TEntity))).AccessInfo.GetDa();
            }
            else
            {
                da = (await EntitySchema.Get(typeof(TEntity))).AccessInfo.GetDa();
            }
            return await da.NewID();
        }
#endif
...
#else
            seq = await model.AccessInfo.GetDa().NewSeq(seqName);
#endif
            Throw.If(seq == 0, $"序列【{seqName}】不存在，请在数据库中手动添加！");
            return seq;
        }
        #endregion
