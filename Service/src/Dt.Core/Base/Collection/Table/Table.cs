...
namespace Dt.Core
{
    ...
    public partial class Table : ObservableCollection<Row>, IRpcJson
    {
        ...
        /// <summary>
        /// 增加行、删除行、单元格值修改后的状态变化事件，调用 LockCollection 后开始记录状态变化
        /// </summary>
        ...
        /// <summary>
        /// 集合和单元格值是否有变化
        /// <para>1. 调用 LockCollection 后开始记录状态，否则无效，UnlockCollection停止记录</para>
        /// <para>2. 在AcceptChanges 或 RejectChanges后重新开始记录</para>
        /// </summary>
        public bool IsDirty
        {
            get { return _isDirty; }
        ...
        /// 获取已被删除的行列表枚举
        /// </summary>
        public IEnumerable<Row> DeletedRows
        {
            get
            {
                if (_lockedList == null)
        ...
        /// 获取是否锁定集合记录集合变化
        /// </summary>
        public bool IsLockedCollection => _lockedList != null;

        void OnTblCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            // 集合是否变化
            bool changed = _lockedList.Count != Count || _lockedList.Except(this).Any();
            if (_isCollectionChanged != changed)
            {
                _isCollectionChanged = changed;
                IsDirty = _isCollectionChanged || _isChanged;
            }
        }
