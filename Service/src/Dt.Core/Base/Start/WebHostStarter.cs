...
namespace Dt.Core
{
    internal static class WebHostStarter
    {
        ...
        static void Configure(IApplicationBuilder p_app)
        {
            // 添加中间件，注意先后顺序！
                ...
                if ("https".Equals(scheme, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    p_options.Listen(IPAddress.Parse(address), port, listenOptions =>
                    {
                        ...
                        try
                        {
                            ...
                            {
                                // 加载X509证书
                                listenOptions.UseHttps(X509CertificateLoader.LoadPkcs12FromFile(Path.Combine(Kit.PathBase, "etc/config/", cerFileName), cerPwd));
                            }
                            else
                            {
                                // 无证书使用默认localhost证书
