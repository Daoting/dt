    ...
    public class Uploader
    {
        ...
        ****************************/
        ...
        public async Task Handle()
        {
            ...
            catch (Exception ex)
            {
                ...
                if (sucFiles.Count > 0)
                {
                    ...
                    try
                    {
                        await _da.Exec($"delete from fsm_file where id in ({sb.ToString().TrimStart(',')})");
                    }
                    catch { }
                }
            }
        ...
        async Task<FileDesc> ReceiveFile(FileMultipartSection p_section)
        {
            ...
            try
            {
                if (await _da.GetDbType() == DatabaseType.Oracle)
                {
                    await _da.Exec("insert into fsm_file (id,name,path,\"size\",uploader,info,ctime,downloads) values (:ID,:Name,:Path,:Size,:Uploader,:Info,:Now,0)", desc);
                }
                else
                {
                    await _da.Exec("insert into fsm_file (id,name,path,size,uploader,info,ctime,downloads) values (@ID,@Name,@Path,@Size,@Uploader,@Info,@Now,0)", desc);
